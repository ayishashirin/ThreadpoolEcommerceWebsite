...
module.exports = {
  addReferralOffer: async (req, res) => {
    try {
      ...
      if (
        req.body?.referralRewards &&
        req.body?.referredUserRewards &&
        req.body?.referralRewards <= req.body?.referredUserRewards
      ) {
      ...
      if (
        req.session.expiry ||
        req.session.discription ||
        req.session.referralRewards ||
        req.session.referredUserRewards
      ) {
  ...
  updateReferralOffer: async (req, res) => {
    try {
      ...
      if (
        req.body?.referralRewards &&
        req.body?.referredUserRewards &&
        Number(req.body?.referralRewards) <=
          Number(req.body?.referredUserRewards)
      ) {
        req.session.referralRewards = `Referral Price Need to be greater`;
      }
      ...
      await adminHelper.updateReferralOffer(
